...
# ─────────────────────────────────────────────────────────
#  AUTOPILOT SPEED
#  FIX: TURBO_STEPS reduced from 2000 → 50 to prevent CPU lag
# ─────────────────────────────────────────────────────────
_AUTO_SPEED = [700]

TURBO_STEPS = 50   # ← FIX: was 2000, caused severe CPU lag at 60fps

def auto_interval(spd):
    """Returns (interval, steps_per_frame).
    Below 1000: 1 step every N frames. At 1000: TURBO (capped at 50 steps/frame)."""
    if spd >= 1000:
...
class Game:
    ...
    def _steps_this_frame(self):
        ...
        Hard cap of 100 prevents any accidental lag spikes."""
        if self.autopilot:
            interval, per_frame = auto_interval(self.auto_speed)
            if per_frame > 1:
                return min(per_frame, 100)  # ← FIX: hard safety cap
