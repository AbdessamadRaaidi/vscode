...
def bfs(start, goals, blocked, cols, rows, wrap=False):
    """BFS from start to any goal. Returns first step direction or None."""
    if not goals: return None
    goal_set = set(goals)
    if start in goal_set: return None
    visited = {start}
    # queue: (pos, first_direction)
    q = deque()
    for d in ALL_DIRS:
        nx, ny = start[0]+d[0], start[1]+d[1]
        if wrap:
            nx %= cols; ny %= rows
        if not wrap and not (0 <= nx < cols and 0 <= ny < rows): continue
...
def flood_fill_size(start, blocked, cols, rows, wrap=False):
    """Count reachable cells from start (for safety check)."""
    visited = {start}; q = deque([start])
    while q:
        x, y = q.popleft()
        for dx, dy in ALL_DIRS:
            nx, ny = x+dx, y+dy
            if wrap: nx %= cols; ny %= rows
            if not wrap and not (0 <= nx < cols and 0 <= ny < rows): continue
            if (nx, ny) in blocked or (nx, ny) in visited: continue
...
def autopilot_dir(snake, food, walls, puf, bonus, enemies, mode, effects, shields, current_dir):
    """
    Decide the best direction for the snake.
    Priority:
      1. Grab a nearby good powerup (speed, double, shield, ghost) if reachable
      2. Avoid bad powerups (slow, shrink) — never targeted
      3. Path to food via BFS
      4. Fallback: largest open space (survival move)

    Blocked set = walls + FULL snake body (conservative — we don't know if we'll
    eat mid-path, so we never assume any body segment will free up).
    """
    head = snake[0]
    ss = set(snake)          # full body, all cells treated as obstacles
    wrap = (mode == "portal")
    enemy_cells = {(e.x, e.y) for e in enemies}

    # Conservative blocked: never remove tail — safer than premature removal
    blocked = walls | ss | enemy_cells

    # --- Score powerup targets ---
    targets = []
    PU_SCORE = {"double": 100, "shield": 90, "speed": 80, "ghost": 70}
    now = time.time()
    for pu_pos, pu_type, pu_time in puf:
        age = now - pu_time
        if pu_type in GOOD_PU and age < 18:
            targets.append((pu_pos, PU_SCORE.get(pu_type, 50)))
        # BAD_PU are never added → never targeted

    # Bonus foods (high value)
    ...
    # Sort targets by score descending, try to BFS to each
    targets.sort(key=lambda x: -x[1])

    for target_pos, _ in targets:
        d = bfs(head, [target_pos], blocked, COLS, ROWS, wrap)
        if d is None: continue
        # Safety check: after stepping in direction d, how much space do we have?
        nx, ny = head[0]+d[0], head[1]+d[1]
        if wrap: nx %= COLS; ny %= ROWS
        # Simulate: new body = head moved, tail stays (worst case — we might eat)
        sim_blocked = walls | ss | enemy_cells  # same conservative set
        free = flood_fill_size((nx, ny), sim_blocked - {(nx, ny)}, COLS, ROWS, wrap)
        if free >= max(8, len(snake)):          # need at least as much space as body
            return d

    # Fallback: pick the direction with the most open space (survival)
    best_d = None; best_free = -1
    for d in ALL_DIRS:
        if d == OPPOSITE.get(current_dir): continue  # never reverse into self
        nx, ny = head[0]+d[0], head[1]+d[1]
        if wrap: nx %= COLS; ny %= ROWS
        if not wrap and not (0 <= nx < COLS and 0 <= ny < ROWS): continue
        if (nx, ny) in blocked: continue
        free = flood_fill_size((nx, ny), blocked - {(nx, ny)}, COLS, ROWS, wrap)
...
class Game:
    ...
    def update(self):
        ...
        if self.frame%self._eff_spd()==0:
            # ── AUTOPILOT DECISION (before dir is committed) ──
            if self.autopilot:
                self._run_autopilot()
            self.dir=self.pend
            ...
            while result=="playing":
                ...
                for ev in pygame.event.get():
                    ...
                    if ev.type==pygame.KEYDOWN:
                        ...
                        elif k==pygame.K_f: g.toggle_autopilot()   # ← AUTOPILOT TOGGLE
