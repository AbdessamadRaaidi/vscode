...
# ─────────────────────────────────────────────────────────
_AUTO_SPEED = [100]   # default 100 out of 1000

def auto_interval(spd):
    """Convert UI speed (1-1000) to frame interval between moves.
    Uses log scale: speed 1 = ~200 frames/move, speed 1000 = 1 frame/move.
    This makes the low end usable for watching and the high end truly fast."""
    t = clamp(spd, 1, 1000) / 1000.0          # 0.001 … 1.0
    # exponential: interval = round(200 * (1/200)^t) = 200^(1-t)
    return max(1, round(200 ** (1.0 - t)))
...
class Game:
    ...
    def _eff_spd(self):
        if self.autopilot:
            return auto_interval(self.auto_speed)
        s=self.move_spd
    ...
    def draw(self,surf,scanlines,fps_on,fps_val,hi,muted):
        ...
        # Speed bar (always visible when autopilot section is shown)
        spd_label = F_XS.render(f"SPD [{self.auto_speed}]  < / >", True, AUTO_COL if self.autopilot else GRAY)
        ...
        # filled portion — color shifts green→yellow→red with speed
        ...
        # store bar rect on surf for mouse hit-test (attached to game object)
            ...
            while result=="playing":
                ...
                for ev in pygame.event.get():
                    ...
                    # ── MOUSE: click/drag speed bar ──
                        ...
                        if bar and bar.collidepoint(ev.pos):
                            dragging_spd = True
