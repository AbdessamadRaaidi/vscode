...
def autopilot_dir(snake, food, walls, puf, bonus, enemies, mode, effects, shields, current_dir):
    ...
    Priority:
      ...
      2. Avoid bad powerups (slow, shrink)
    ...
    # Build blocked set (walls + snake body + enemies)
    # We keep tail unblocked since it will move — treat last segment as free
    tail = snake[-1]
    body_blocked = ss - {tail}
    blocked = walls | body_blocked | enemy_cells

    # --- Score powerup targets ---
    targets = []

    # Good powerups: prioritize double > shield > speed > ghost
    PU_SCORE = {"double": 100, "shield": 90, "speed": 80, "ghost": 70}
    now = time.time()
    for pu_pos, pu_type, pu_time in puf:
        age = now - pu_time
        if pu_type in GOOD_PU:
            # Only grab if still has decent time left (> 10s)
            if age < 18:
                targets.append((pu_pos, PU_SCORE.get(pu_type, 50)))
        # Avoid bad powerups by never targeting them
    ...
    for target_pos, _ in targets:
        ...
        # Safety check: make sure the chosen move doesn't trap us
        nx, ny = head[0]+d[0], head[1]+d[1]
        if wrap: nx %= COLS; ny %= ROWS
        # Simulate moving: new blocked = walls + (body minus tail) + new head
        sim_blocked = walls | (ss - {tail}) | enemy_cells
        free = flood_fill_size((nx, ny), sim_blocked, COLS, ROWS, wrap)
        if free >= max(5, len(snake) // 2):
            return d

    # Fallback: pick the direction with the most open space (survival)
    best_d = None; best_free = -1
    for d in ALL_DIRS:
        if d == OPPOSITE.get(current_dir): continue  # don't reverse
        nx, ny = head[0]+d[0], head[1]+d[1]
        if wrap: nx %= COLS; ny %= ROWS
        if not wrap and not (0 <= nx < COLS and 0 <= ny < ROWS): continue
        if (nx, ny) in blocked: continue
...
class Game:
    ...
    def update(self):
        ...
        # ── AUTOPILOT DECISION ──
        if self.autopilot and self.frame % self._eff_spd() == 1:
            self._run_autopilot()
