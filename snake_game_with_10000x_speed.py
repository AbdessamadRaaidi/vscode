...
def auto_interval(spd):
    ...
    Below 1000: normal 1 step every N frames.
    At 1000: TURBO — run 500 steps per frame (effectively 30000 moves/sec)."""
    if spd >= 1000:
        return (1, 500)   # turbo: 500 moves per frame
...
class Game:
    ...
    def _do_step(self, now):
        """Execute one snake move. Returns 'dead' or None."""
        if self.autopilot:
            self._run_autopilot()
        self.dir=self.pend
    ...
    def draw(self,surf,scanlines,fps_on,fps_val,hi,muted):
        ...
        spd_label_txt = "⚡ TURBO x500" if is_turbo else f"SPD [{self.auto_speed}]  < / >"
        spd_label = F_XS.render(spd_label_txt, True, (255,80,0) if is_turbo else (AUTO_COL if self.autopilot else GRAY))
        ...
        if is_turbo:
            # full bar pulsing orange-red for turbo
        ...
        yo += bar_h + 4
        # TURBO button
        turbo_col = (255,80,0) if is_turbo else (45,48,65)
        turbo_txt = F_XS.render("[ T ] TURBO MODE", True, (255,160,0) if is_turbo else GRAY)
        pygame.draw.rect(surf, turbo_col, (bar_x, yo, bar_w, 16), border_radius=3)
        surf.blit(turbo_txt, turbo_txt.get_rect(center=(bar_x + bar_w//2, yo+8)))
        self._turbo_rect = pygame.Rect(bar_x, yo, bar_w, 16)
        yo += 20
